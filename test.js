...
export const lineupsSoccer365 = async (href) => {
    ...
    const options = {
        method: 'GET',
        url: `https://cappers.pro/prognozy/na-futbol?data=now`,
        headers: {
    ...
    try {
        ...
        let arrEl = dom.window.document.querySelectorAll('.card prognozy-article')

        arrEl.forEach(el => {
                links.push(`${el.querySelector('a').getAttribute('href').trim()}`);         
        })
    ...
    try {
        ...
        result.forEach(el => {
            ...
            matches.push({
                ...
                // text: block.querySelector('.forecast-text').textContent.trim(),
